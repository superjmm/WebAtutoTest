...
import allure
import logging
from Operate.backstage_manage import OperateManage
...
add_path = base_path+'/Case/setup-operate-add.xlsx' #添加测试用例
update_path = base_path+'/Case/setup-operate-update.xlsx' #编辑测试用例
delete_path = base_path+'/Case/setup-operate-delete.xlsx' #删除测试用例
...
def before():
    ...
    初始化类
    """    
    global operate_manage
    operate_manage = OperateManage()
...
class TestOperate():    
    ...
    def test_add(self,case):
        ...
        if case_id is None: #用例编号不能为空
            ...
            result = operate_manage.add(case)
            if result:
    ...
    def test_update(self,case):
        ...
        if case_id is None: #用例编号不能为空
            ...
            result = operate_manage.update(case)
            if result:
    ...
    def test_delete(self,case):
        ...
        if case_id is None: #用例编号不能为空
            ...
            result = operate_manage.delete(case)
            if result:
