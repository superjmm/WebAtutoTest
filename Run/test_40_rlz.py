...
class TestRlz():                
    ...
    @pytest.mark.parametrize('case',update_gather_list)
    @allure.feature('热力站-采集量编辑点地址')
    def test_update_gather(self,case):
        global rlz_manage
        global add_row
        global logger
        add_row += 1  
        ...
        # 执行用例
            ...
            result = rlz_manage.update_gather_param(case)
            if result:
                handle_excel.write_value(update_gather_path,n,test_result_col,'通过')
            else:
                handle_excel.write_value(update_gather_path,n,test_result_col,'失败')
            assert result
